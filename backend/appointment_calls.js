...
// ---------------------------------------------------------------------
// Unauthenticated webhooks — Twilio itself calls these while the phone
// call is live. Protected only by the call's own unguessable UUID (Twilio
// can't send our app's login token), same trust model as e.g. Stripe
// webhook URLs with a secret path component.
// ---------------------------------------------------------------------
...
async function respondForTurn(req, res, id) {
  ...
  if (speechResult) transcript.push({ role: 'them', text: speechResult });

  const turnCount = call.turn_count + (speechResult ? 1 : 0);
  ...
  if (forceWrapUp) {
    ...
    turn = await nextTurn(call, transcript);
  }
  ...
  if (turn.done) {
    ...
    await pool.query(
      `UPDATE appointment_calls SET
       ...
       WHERE id = $3`,
      [JSON.stringify(transcript), turnCount, id]
    );
  }
  ...
  try {
    ...
    if (call && !['completed', 'canceled'].includes(call.status)) {
      const statusMap = {
        ...
        failed: 'failed',
        canceled: 'canceled',
        completed: call.outcome ? 'completed' : 'completed', // already set by respondForTurn if we hung up
      };
